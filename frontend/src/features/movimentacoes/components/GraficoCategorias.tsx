...
export function GraficoCategorias({ movimentacoes }: GraficoCategoriasProps) {
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-3 shadow-lg text-xs space-y-1 relative z-50 opacity-100">
  ...
  return (
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 shadow-sm space-y-4 flex flex-col justify-between h-full transition-colors">
      ...
      {data.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center text-center text-xs text-zinc-400 dark:text-zinc-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
          ...
          <div className="sm:col-span-6 space-y-2 max-h-56 overflow-y-auto pr-1">
            {data.map((item, index) => (
              <div
                key={item.name}
                className="flex items-center justify-between text-xs p-1.5 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800/60 transition-colors"
              >
